...
func (i *OpusReader) getPage() (uint8, error) {
	...
	if bytes.Compare(head, []byte("OggS")) != 0 {
		return 0, fmt.Errorf("Incorrect page. Does not start with \"OggS\" : %s %v", string(head), hex.EncodeToString(head))
	}
	//Skipping Version
	...
	if bytes.Compare(magic, []byte("OpusHead")) == 0 {
		...
		io.CopyN(ioutil.Discard, i.stream, int64(payloadLen-8))
	}
